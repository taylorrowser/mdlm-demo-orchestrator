...
Every runner-owned process now has a durable command journal in addition to the legacy raw evidence triplet. Before spawn, the runner atomically writes and syncs `command-NNNNNN.intent.json` with argv, working directory, timeout, input identity, and environment names. When the child closes, it atomically writes and syncs `command-NNNNNN.completion.json`; that one document binds the intent and contains the full process record, including raw stdout and stderr as Base64. Only then does the runner write and sync the separate `.stdout`, `.stderr`, and `.json` compatibility triplet. A parent crash therefore leaves either an intent with an uncertain child outcome, or one complete atomic child result that a recovery path can authenticate without reconstructing output from process text. The intent proves what the runner authorized before spawn. It does not claim the child never started. Before another command starts, the runner validates every settled intent, completion, and compatibility triplet. An incomplete or altered journal stops the run instead of repeating a child whose outcome is uncertain.
